...
export default function ExamList() {
  ...
  const fieldCode = searchParams.get('fieldCode') || 'all';
  const sort = searchParams.get('sort') || 'endDate,asc';
  const page = parseInt(searchParams.get('page') || '0');

  // 2. 필터 변경 함수 (페이지 이동 버그 수정)
  const handleFilterChange = (key: string, value: string) => {
    ...
    // 페이지 이동(page)이 아닌 '필터'를 바꿀 때만 0페이지로 보냅니다.
    if (key !== 'page') {
  ...
  const mapStatusForApi = (uiStatus: string) => {
    if (uiStatus === '접수중') return 'active';
    if (uiStatus === '접수예정') return 'upcoming';
    return '';
  };
    ...
    const fetchExamsData = async () => {
      try {
        ...
        const response: PageResponse = await getExams({
          ...
          status: mapStatusForApi(statusParam),
          fieldCode: fieldCode === 'all' ? undefined : fieldCode,
          page,
          size: 12,
          sort
        });
        ...
        if (response && response.content) {
          const mapped = response.content.map((dto): Exam => {
            const today = getDate(new Date()).getTime();
            const start = new Date(dto.startDate).getTime();
            const end = new Date(dto.endDate).getTime();

            // ExamStatus 타입 준수 ('접수마감' 제거)
            const statusLabel: ExamStatus = (today >= start && today <= end) ? '접수중' : '접수예정';
  ...
  // 4. 대직무분야 정식 라벨링 (24종 + 기타)
  const categoryOptions = [
    { label: '전체 분야', value: 'all' },
    { label: '사업관리', value: '01' }, { label: '경영.회계.사무', value: '02' },
    { label: '교육.자연.과학.사회과학', value: '04' }, { label: '보건.의료', value: '06' },
    { label: '사회복지.종교', value: '07' }, { label: '문화.예술.디자인.방송', value: '08' },
    { label: '운전.운송', value: '09' }, { label: '영업.판매', value: '10' },
    { label: '이용.숙박.여행.오락.스포츠', value: '12' }, { label: '음식서비스', value: '13' },
    { label: '건설', value: '14' }, { label: '광업자원', value: '15' },
    { label: '기계', value: '16' }, { label: '재료', value: '17' },
    { label: '화학', value: '18' }, { label: '섬유.의복', value: '19' },
    { label: '전기.전자', value: '20' }, { label: '정보통신', value: '21' },
    { label: '식품.가공', value: '22' }, { label: '인쇄.목재.가구.공예', value: '23' },
    { label: '농림어업', value: '24' }, { label: '안전관리', value: '25' },
    { label: '환경.에너지', value: '26' }, { label: '기타', value: '00' }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <FilterSelect label="직무 분야" value={fieldCode} onChange={(v: string) => handleFilterChange('fieldCode', v)} options={categoryOptions} />
        <FilterSelect label="진행 상태" value={statusParam} onChange={(v: string) => handleFilterChange('status', v)} options={['all', '접수중', '접수예정']} />
        <FilterSelect label="정렬 방식" value={sort} onChange={(v: string) => handleFilterChange('sort', v)} options={['endDate,asc', 'itemName,asc']} isSort />
      </div>

      {isLoading ? (
        <div className="py-20 text-center"><Loader2 className="animate-spin mx-auto w-10 h-10 text-blue-600" /></div>
      ) : pageResponse?.empty ? (
        <div className="text-center py-20 bg-white rounded-2xl border">결과가 없습니다.</div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {exams.map(exam => <ExamCard key={exam.itemCode + exam.type} exam={exam} />)}
          </div>
          <div className="flex justify-center items-center space-x-4 mt-12">
            <button onClick={() => handleFilterChange('page', String(page - 1))} disabled={pageResponse?.first} className="p-2 disabled:text-gray-300 text-blue-600"><ChevronLeft /></button>
            <span className="font-bold">{(pageResponse?.number || 0) + 1} / {pageResponse?.totalPages || 1}</span>
            <button onClick={() => handleFilterChange('page', String(page + 1))} disabled={pageResponse?.last} className="p-2 disabled:text-gray-300 text-blue-600"><ChevronRight /></button>
          </div>
        </>
      )}
    </div>
...
function FilterSelect({ label, value, options, onChange, isSort = false }: any) {
  return (
    <div>
      <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">{label}</label>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full p-3 bg-white border border-gray-100 rounded-xl outline-none cursor-pointer">
        {options.map((opt: any) => {
          const val = typeof opt === 'object' ? opt.value : opt;
          const lbl = typeof opt === 'object' ? opt.label : opt;
          return (
            <option key={val} value={val}>
              {isSort ? (val.includes('endDate') ? '시험일 순' : '이름 순') : (lbl === 'all' ? '전체' : lbl)}
            </option>
          );
        })}
      </select>

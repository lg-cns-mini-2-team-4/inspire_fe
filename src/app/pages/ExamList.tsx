...
import { ExamCard } from '../components/ExamCard';
import { Exam, ExamCategory, ExamStatus } from '../types/exam';
import { Search, Calendar as CalendarIcon, List as ListIcon, Loader2, RotateCcw } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchExamsData = async () => {
      try {
        ...
        // 백엔드 파라미터 규격에 맞게 변환
        // status: "active" (접수중), "upcoming" (접수예정)
        const apiStatus = selectedStatus === '접수중' ? 'active' : selectedStatus === '접수예정' ? 'upcoming' : undefined;

        const response: PageResponse = await getExams({
          itemName: searchQuery || undefined,
          status: apiStatus,
          // fieldCode는 필요시 카테고리 이름과 코드를 매핑하는 로직 추가 가능
          size: 100, // 전체보기를 위해 일단 넉넉히 가져옴
        });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <FilterSelect label="직무 분야" value={selectedCategory} options={categories} onChange={(v) => setSelectedCategory(v as any)} />
          <FilterSelect label="진행 상태" value={selectedStatus} options={statuses} onChange={(v) => setSelectedStatus(v as any)} />
          <FilterSelect label="정렬 방식" value={sortBy} options={['date', 'name']} onChange={(v) => setSortBy(v as any)} isSort />
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-20 text-center">
          <p className="text-gray-400 font-medium font-sans">조건에 맞는 시험 결과가 없습니다.</p>
          <button 

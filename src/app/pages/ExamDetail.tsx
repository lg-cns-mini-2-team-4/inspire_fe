...
// 1. 작성해두신 certApi import
import { certApi } from '../api/certApi';

// 2. 백엔드 응답에 맞춘 새로운 DTO 규격 (mediumFieldName 삭제, 날짜 null 가능성 반영)
interface ScheduleInfo {
  implYear: string;
  implSeq: number;
  description: string;
...
interface ExamDetailResponseDTO {
  itemCode: string;
  itemName: string;
  certTypeName: string;
  seriesName: string;
...
export default function ExamDetail() {
  const { id } = useParams(); // itemCode 역할
  const { getExamDetail } = certApi();

  const [data, setData] = useState<ExamDetailResponseDTO | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  // 3. 커스텀 훅을 활용한 데이터 Fetching 로직 (즐겨찾기, useAuth 모두 제거됨)
  useEffect(() => {
    const fetchDetail = async () => {
      if (!id) return;
...
// 날짜 행 렌더링을 위한 헬퍼 컴포넌트 (값이 없을 경우 렌더링하지 않음)
function DateRow({ 
  label, 
  start, 
  end, 
  ...
  return (
    <div className="flex justify-between items-center text-sm">
      ...
      <span className={`font-bold ${highlight ? color : 'text-gray-800'}`}>
        {isSingle 
          ? format(new Date(start), 'yyyy. MM. dd') 
          : `${format(new Date(start), 'MM. dd')} ~ ${end ? format(new Date(end), 'MM. dd') : ''}`
        }
      </span>
    </div>
  );

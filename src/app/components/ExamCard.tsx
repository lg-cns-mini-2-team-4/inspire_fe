...
import { Link } from 'react-router';
import { Exam } from '../types/exam';
import { Calendar, Clock, Building2, MapPin, Star, Info } from 'lucide-react';
import { format } from 'date-fns';
import { ko } from 'date-fns/locale';
import { useAuth } from '../context/AuthContext';
import { useFavorites, useToggleFavorite } from '../hooks/favorite';
...
export function ExamCard({ exam }: ExamCardProps) {
  ...
  // 1. 내 즐겨찾기(일정) 목록 가져오기
  const { data: favorites = [] } = useFavorites(today);
  
  // 2. 즐겨찾기 토글(추가/삭제) 기능 가져오기
  const toggleMutation = useToggleFavorite();

  // 3. 현재 이 카드가 내 일정에 있는지 확인 (refId와 itemCode 매칭)
  const favoriteItem = favorites.find((fav: any) => fav.refId === exam.itemCode);
  const isFavorite = !!favoriteItem; // 존재하면 true
  ...
  const handleFavoriteClick = (e: React.MouseEvent) => {
    ...
    // 4. Mutation 실행 (등록 시 데이터 가공, 삭제 시 ID 전달)
    toggleMutation.mutate({
      exam: exam,
      isFavorite: isFavorite,
      scheduleId: favoriteItem?.id // 🗑️ 삭제할 때 필요한 고유 번호 (Long)
    });
  };

  return (
    <Link 
      to={`/exam/${exam.itemCode}`} 
      className="block bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-all relative group"
    >
      {/* 상단 배지 및 별표 버튼 */}
      <div className="flex justify-between items-start mb-4">
        <div className="flex gap-2">
          <span className="px-2 py-1 bg-gray-50 text-gray-500 text-[11px] font-bold rounded border border-gray-100">
            {exam.category || '국가기술'}
          </span>
          <span className={`px-2 py-1 text-[11px] rounded font-bold ${
            exam.status === '접수중' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'
          }`}>
            {exam.status}
          </span>
        </div>
        ...
        {accessToken && (
          <button 
            ...
            disabled={toggleMutation.isPending} // 💡 통신 중에는 버튼 비활성화
            className={`p-1.5 rounded-full transition-colors ${
              isFavorite ? 'text-yellow-400' : 'text-gray-300 hover:bg-gray-50'
            }`}
          >
            <Star className={`w-5 h-5 ${isFavorite ? 'fill-current' : ''}`} />
          </button>
      ...
      {/* 시험 제목 */}
      <h3 className="text-lg font-bold text-gray-900 mb-5 leading-snug group-hover:text-blue-600 transition-colors">
        {exam.itemName}
      </h3>
      ...
      <div className="space-y-3 mb-6">
        <div className="flex items-center gap-3 text-gray-600">
          ...
          <span className="text-sm font-medium">
            시험일: {exam.startDate ? format(new Date(exam.startDate), 'yyyy. MM. dd (eee)', { locale: ko }) : '확인 중'}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 text-gray-600">
          ...
          <span className="text-sm font-medium">
            접수: {exam.startDate ? format(new Date(exam.startDate), 'M/d') : '-'} ~ {exam.endDate ? format(new Date(exam.endDate), 'M/d') : '-'}
          </span>
        </div>

        <div className="flex items-center gap-3 text-gray-600">
          <Info className="w-4 h-4 text-gray-400" />
          <span className="text-sm font-medium">구분: {exam.type === 'WR' ? '필기' : '실기'}</span>
        </div>
      </div>

      {/* 설명 */}
      {exam.description && (
        <div className="pt-4 border-t border-gray-50 text-xs text-gray-400 truncate italic">
          "{exam.description}"
        </div>
      )}
    </Link>
